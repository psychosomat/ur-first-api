...
2. Введите URL: `https://api.spacexdata.com/v4/planets`
...
- Статус: `200 OK`
- Тело ответа (JSON): список планет с параметрами
```json
[
  {
    "id": "earth",
...
1. Метод: **POST**
2. URL: `https://api.nasa.gov/v3/planets` (пример)
3. В Headers:
   ```json
   {
...
```javascript
// Проверка статуса
pm.test("Миссия успешна", () => {
    pm.response.to.have.status(201);
});

// Проверка структуры ответа
pm.test("Планета создана", () => {
    const response = pm.response.json();
    pm.expect(response).to.have.property("id");
    pm.expect(response.name).to.eql("Proxima Centauri b");
});
```
...
**Поздравляем с завершением Главы 1!** Вы освоили основы работы с API. В следующих главах мы построим собственный "космический корабль" — веб-приложение, использующее космические API!
